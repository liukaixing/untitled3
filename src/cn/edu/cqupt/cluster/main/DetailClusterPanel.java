...
class DetailClusterPanel {
	...
	static public JPanel tmpPanel1 = new JPanel();
	static int pageNum = 1;// 默认设置当前页为第一页
	int pageSize; // 每页显示的记录数
	String[] columnName; // 列名
	List<Spectrum> sourceDataList; // 一个聚类结果文件中所有的数据
	...
	public DetailClusterPanel(int pageSize, String[] columnName, List<Spectrum> sourceDataList)
			throws Exception {
	...
	public void printNextPage() throws Exception {
		...
		List<Spectrum> result = (List<Spectrum>) page.getDataList(); // 获取待显示的数据
...
class DetailClusterPanel2 {
	...
	public DetailClusterPanel2(int pageSize, String[] columnName, List<Spectrum> sourceDataList)
			throws Exception {
		this.pageSize = pageSize;
		this.columnName = columnName;
		this.sourceDataList = sourceDataList;
...
class DetailClusterTableModel extends AbstractTableModel {
	...
	public DetailClusterTableModel(List<Spectrum> spectra, String[] columnName) throws Exception {
		this.spectra = spectra; // 所有聚类结果
		this.rowCount = spectra.size(); // 行数
	...
	public Object getValueAt(int r, int c) {
		...
		switch (c) {
		case 0:
			value = spectra.get(r).getSpectrumId();
			break;
		case 1:
			value = spectra.get(r).getPrecursorMz();
			break;
		case 2:
			value = spectra.get(r).getCharge();
			break;
		case 3:
			value = spectra.get(r).getSimilarityScore();
			break;
		case 4:
			value = spectra.get(r).getSpecies();
		}
		// } catch (Exception e) {
		// e.printStackTrace();
		// }
